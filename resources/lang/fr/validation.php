<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Validation Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines contain the default error messages used by
    | the validator class. Some of these rules have multiple versions such
    | as the size rules. Feel free to tweak each of these messages here.
    |
    */

    'accepted' => 'L\' :attribute doit être accepté.',
    'active_url' => 'L\' :attribute n\'est pas une URL valide.',
    'after' => 'L\' :attribute doit être une date postérieure à :date.',
    'after_or_equal' => 'L\' :attribute doit être une date postérieure ou égale à :date.',
    'alpha' => 'L\' :attribute ne peut contenir que des lettres.',
    'alpha_dash' => 'L\' :attribute ne peut contenir que des lettres, des chiffres, des tirets et des traits de soulignement.',
    'alpha_num' => 'L\' :attribute ne peut contenir que des lettres et des chiffres.',
    'array' => 'L\' :attribute doit être un tableau.',
    'before' => 'L\' :attribute doit être une date antérieure à :date.',
    'before_or_equal' => 'L\' :attribute doit être une date antérieure ou égale à :date.',
    'between' => [
        'numeric' => 'L\' :attribute doit être compris entre :min et :max.',
        'file' => 'L\' :attribute doit être compris entre :min et :max kilo-octets.',
        'string' => 'L\' :attribute doit être compris entre les caractères :min et :max.',
        'array' => 'L\' :attribute doit être compris entre les éléments :min et :max.',
    ],
    'boolean' => 'Le champ du :attribute doit être vrai ou faux.',
    'confirmed' => 'La confirmation du :attribute ne correspond pas.',
    'date' => 'L\' :attribute n\'est pas une date valide.',
    'date_equals' => 'L\' :attribute doit être une date égale à :date.',
    'date_format' => 'L\' :attribute ne correspond pas au format :format.',
    'different' => 'L\' :attribute et :other doivent être différents.',
    'digits' => 'L\' :attribute doit être :digits digits.',
    'digits_between' => 'L\' :attribute doit être compris entre les chiffres :min et :max.',
    'dimensions' => 'L\' :attribute a des dimensions d\'image non valables.',
    'distinct' => 'Le champ :attribute a une valeur fr double.',
    'email' => 'L\' :attribute doit être une adresse email valide.',
    'ends_with' => 'L\' :attribute doit se terminer par l\'un des éléments suivants : valeurs :.',
    'exists' => 'L\' :attribute sélectionné n\'est pas valide.',
    'file' => 'L\' :attribute doit être un fichier.',
    'filled' => 'Le champ :attribute doit avoir une valeur.',
    'gt' => [
        'numeric' => 'L\' :attribute doit être supérieur à la valeur :.',
        'file' => 'L\' :attribute doit être supérieur à la valeur :kilo-octets.',
        'string' => 'L\' :attribute doit être supérieur aux caractères :value.',
        'array' => 'L\' :attribute doit être supérieur à :valeur éléments.',
    ],
    'gte' => [
        'numeric' => 'L\' :attribute doit être supérieur ou égal à :value.',
        'file' => 'L\' :attribute doit être supérieur ou égal à :valeur kilo-octets.',
        'string' => 'L\' :attribute doit être supérieur ou égal à :valeur caractères.',
        'array' => 'L\' :attribute doit comporter au moins des éléments :value.',
    ],
    'image' => 'L\' :attribute doit être une image.',
    'in' => 'L\' :attribute sélectionné n\'est pas valide.',
    'in_array' => 'Le champ :attribute n\'existe pas dans :other.',
    'integer' => 'L\' :attribute doit être un nombre entier.',
    'ip' => 'L\' :attribute doit être une adresse IP valide.',
    'ipv4' => 'L\' :attribute doit être une adresse IPv4 valide.',
    'ipv6' => 'L\' :attribute doit être une adresse IPv6 valide.',
    'json' => 'L\' :attribute doit être une chaîne JSON valide.',
    'lt' => [
        'numeric' => 'L\' :attribute doit être inférieur à la valeur :.',
        'file' => 'L\' :attribute doit être inférieur à la valeur :kilo-octets.',
        'string' => 'L\' :attribute doit être inférieur à :value caractères.',
        'array' => 'L\' :attribute doit comporter moins de :valeurs.',
    ],
    'lte' => [
        'numeric' => 'L\' :attribute doit être inférieur ou égal à :value.',
        'file' => 'L\' :attribute doit être inférieur ou égal à :valeur kilo-octets.',
        'string' => 'L\' :attribute doit avoir une valeur inférieure ou égale à :caractères de valeur.',
        'array' => 'L\' :attribute ne doit pas comporter plus d\'éléments :value.',
    ],
    'max' => [
        'numeric' => 'L\' :attribute ne doit pas être supérieur à :max.',
        'file' => 'L\' :attribute ne peut pas être supérieur à :max kilo-octets.',
        'string' => 'L\' :attribute ne peut pas être supérieur à :max caractères.',
        'array' => 'La :attribute ne peut pas avoir plus de :max choix.',
    ],
    'mimes' => 'L\' :attribute doit être un fichier de type : :valeurs.',
    'mimetypes' => 'L\' :attribute doit être un fichier de type : :valeurs.',
    'min' => [
        'numeric' => 'Le :attribute doit être au moins de type :min.',
        'file' => 'Le :attribute doit être au moins :min kilo-octets.',
        'string' => 'Le :attribute doit être au moins de :min caractères.',
        'array' => 'Le :attribute doit comporter au moins des éléments de type :min.',
    ],
    'not_in' => 'L\' :attribute sélectionné n\'est pas valide.',
    'not_regex' => 'Le format du :attribute n\'est pas valide.',
    'numeric' => 'L\' :attribute doit être un nombre.',
    'password' => 'Le mot de passe est incorrect.',
    'present' => 'Le champ :attribute doit être présent.',
    'regex' => 'Le format du :attribute n\'est pas valide.',
    'required' => 'Le champ :attribute est requis.',
    'required_if' => 'Le champ :attribute est requis lorsque :other est une valeur.',
    'required_unless' => 'Le champ :attribute est requis sauf si :other est dans :values.',
    'required_with' => 'Le champ :attribute est requis lorsque :values est présent.',
    'required_with_all' => 'Le champ :attribute est requis lorsque des valeurs :sont présentes.',
    'required_without' => 'Le champ d\' :attribute est requis lorsque :values n\'est pas présent.',
    'required_without_all' => 'Le champ d\' :attribute est requis lorsqu\'aucune des valeurs :n\'est présente.',
    'same' => 'L\' :attribute et :other doivent correspondre.',
    'size' => [
        'numeric' => 'L\' :attribute doit être :size.',
        'file' => 'L\' :attribute doit être :size kilo-octets.',
        'string' => 'L\' :attribute doit être un caractère de taille :size.',
        'array' => 'L\' :attribute doit contenir des éléments :size.',
    ],
    'starts_with' => 'L\' :attribute doit commencer par l\'un des éléments suivants : valeurs :.',
    'string' => 'L\' :attribute doit être une chaîne de caractères.',
    'timezone' => 'L\' :attribute doit être une zone valide.',
    'unique' => 'Le champ :attribute a déjà été attribué.',
    'uploaded' => 'L\' :attribute n\'a pas été téléchargé.',
    'url' => 'Le format du :attribute n\'est pas valide.',
    'uuid' => 'L\' :attribute doit être un UUID valide.',

    /*
    |--------------------------------------------------------------------------
    | Custom Validation Language Lines
    |--------------------------------------------------------------------------
    |
    | Here you may specify custom validation messages for attributes using the
    | convention "attribute.rule" to name the lines. This makes it quick to
    | specify a specific custom language line for a given attribute rule.
    |
    */

    'custom' => [
        'attribute-name' => [
            'rule-name' => 'custom-message',
        ],
        'conditions.required' => 'Vous devez être en accord avec les conditions',
    ],
    'attributes' => [
        'title' => 'titre',
        'picture' => 'image',
        'location' => 'localité',
        'job' => 'métier',
        'category-job' => 'catégorie(s) de métier',
        'name' => 'nom',
        'subject' => 'sujet',
        'surname' => 'prénom',
        'disponibility' => 'disponibilité',
        'adress' => 'adresse',
        'password' => 'mot de passe',
        'categoryUser' => 'catégorie de métier',
        'number' => 'téléphone',
        'categoryAds' => 'catégorie de métier',
        'startmonth' => 'mois de commencement',
        'websitetwo' => '2éme site internet',
        'websitethree' => '3éme site internet',
        'locationtwo' => '2éme localité',
        'locationthree' => '3éme localité',
    ],

    /*
    |--------------------------------------------------------------------------
    | Custom Validation Attributes
    |--------------------------------------------------------------------------
    |
    | The following language lines are used to swap our attribute placeholder
    | with something more reader friendly such as "E-Mail Address" instead
    | of "email". This simply helps us make our message more expressive.
    |
    */

];
